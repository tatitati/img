...
object ValidatorEvent {
  ...
  def isNewEventAfterLastEvent(newevent: Event, lastEvent: Option[Event]): Boolean = lastEvent match {
    case Some(last) => last.when < newevent.when
    case None => true
  }

  def goalsAreMovingForward(newevent: Event, lastEvent: Option[Event]): Boolean = lastEvent match {
  ...
  def teamNotScoringRemainTheSame(newevent: Event, lastEvent: Option[Event]): Boolean = lastEvent match {
